...
class ServiceClient {
  constructor(options) {
    ...
    this.responseWrapper = _.isNil(options.responseWrapper) ? true : !!options.responseWrapper;
  }
  ...
  request(path, options, callback) {
    ...
    const result = getSteamData.then((streamData) => {
      ...
      if (callback) {
        cb = (err, body, res) => {
          ...
          if (this.responseWrapper && body && body.code !== 'SUCCESS') {
            return callback(body);
          }

          if (res instanceof http.IncomingMessage) {
            callback(null, this.responseWrapper ? (body && body.data) : body, res);
          } else {
            callback(null, this.responseWrapper ? (body && body.data) : body);
          }
        };
      }
    ...
    if (callback) {
      ...
      return result.then(d => {
        ...
        if (res instanceof http.IncomingMessage) {
          return {
            data: this.responseWrapper ? (body && body.data) : body,
            res
          };
        } else {
          return this.responseWrapper ? (body && body.data) : body;
        }
      });
    }
  }
